...
def get_course_runs(user, course_keys=None):
    ...
    #     course_catalog_data_dict = {
    #         _get_course_key_from_catalog_course_run_cache_key(cached_key): cached_course_run
    #         for cached_key, cached_course_run in cached_course_catalog_data.iteritems()
    #     }
...
class CatalogCourseRunCacheUtility:
    ...
    def get_course_keys_not_found_in_cache(cls, course_keys, cached_course_run_keys):
        """
        Get course keys for which course run data is not available in cache.
        """
        missing_course_keys = course_keys - cached_course_run_keys
        # cached_course_keys = catalog_course_run_against_course_key.keys()
        # for course_key in course_keys:
        #     if course_key in cached_course_keys:
        #         course_keys.remove(course_key)
        #
        log.debug("Catalog data not found in cache against Course Keys: '{}'".format(",".join(missing_course_keys)))
