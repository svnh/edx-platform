...
from openedx.core.djangoapps.catalog.utils import get_run_marketing_urls, get_run_marketing_url
...
def get_link_for_about_page(course_key, user, catalog_course_run=None):
    ...
    if settings.FEATURES.get('ENABLE_MKTG_SITE'):
        if catalog_course_run:
            return catalog_course_run.get('marketing_url')
        return get_run_marketing_url(user, course_key)
