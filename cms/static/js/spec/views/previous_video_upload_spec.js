    ...
    function($, _, Backbone, PreviousVideoUploadView, TemplateHelpers, ViewHelpers) {
        ...
        describe('PreviousVideoUploadView', function() {
            ...
            it('should render remove button correctly', function() {
                ...
                expect(removeButton.find('.sr').text()).toEqual('Remove this video');
            });

            it('shows a confirmation popup when the remove button is clicked', function() {
                var $el = render();
                $el.find('a.remove-video-button').click();
                expect($('.prompt.warning .title').text()).toEqual('Remove Video From List?');
                expect(
                    $('.prompt.warning .message').text()
                ).toEqual(
                    'Are you sure you want to remove this video from the list? Note: Removing a video from this list does not affect course content. Any content that uses a previously uploaded video ID continues to display in the course.'  // eslint-disable-line max-len
                );
